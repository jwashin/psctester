...
##VERSION = 2.19      # add support for Q Family 
VERSION = 2.20      # support new gui 06/07/2023

DOTEST_VERSION = VERSION

with open('do_log', 'w') as f:
    f.write('starting version {}\n'.format(VERSION))
...
def dotest():
    ...
    def log_KWH_results(report_title):
        ...
        if len(DUT.child_data) == 0:
            logfile.write("Channel,Percent Error{},,Within tolerance?\n".format(' (avg)' if (DUT.meter_type != 'QFAM' and DUT.num_active_phases <= 3) else ''))
        ...
        for test_key in ['low', 'high']:
            for data_index in range (len(test_data_log[test_key].phase_data)):
                logfile.write('{},,{},,{},,{},,{},,{},,{}\n'.format(test_data_log[test_key].test_amps, 
                                                                    ...
                                                                    test_data_log[test_key].phase_data[data_index][3], 
                                                                    test_data_log[test_key].phase_data[data_index][4], 
                                                                    test_data_log[test_key].percent_uncertainty
                                                                    ))
                                # phase_data is a list of tuples (per phase)
                                # (test_seconds, radian_KWH,start_reading, end_reading, percent_error)
...
##            return False
    ...
    tester = Tester_hardware_obj(test_type)
    ...
    if not hasattr(Radian,  'model'):
        ...
        time.sleep(5)
        lw.message('done')
        Q_logger.info('Unable to contact Radian')

        with open('do_log', 'a') as f:                              ## TEST!!!
            f.write('no radian/n')
 ...
 #   supply = ACsupply_obj();
    ...
    elif test_type in ['TMX1','TMX3','TMX4', 'QFAM']:
...
#            print(DUT.channel_mask)
        ...
        if Okay:
            ...
            if DUT.meter_type == 'QFAM':
                num_points = DUT.num_phases
            elif DUT.num_phases <= 3:
                num_points = 1
            else:
                num_points = DUT.num_phases // 2
...
for i in range(1):         # test!!!!!!!!!!!!!!!!!!!!!!
    ...
    Q_logger.info('Starting do_test() Version {} on tester# {}'.format(DOTEST_VERSION, tester_number))
