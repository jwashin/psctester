...
.
VERSION = 2.25      # Add support for QFAM
...
from conf import ST4_serno_list, I2C_ADDR
from logwriter import logwriter as lw
from caveman_funcs import range_lookup
from PSC_QFAM_interface import PulseTest
...
class Meter_obj():
    ...
    def __init__(self,ttydev,
        ...
        if self.meter_type == 'QFAM':                                                       # Meter is Q family - use modbus to read
            self.use_current_gating = False
            self.pulse_test = PulseTest(I2C_ADDR, self.ttydev)
            if self.pulse_test.get_meter_info():
                self.meter_serno = '{:08x}'.format(self.pulse_test.meter.serno)
                (self.meter_date, self.meter_time) = self.pulse_test.meter.date_and_time.split()[0:2]
                self.meter_version = '{:08x}'.format(self.pulse_test.meter.version)
                try:
                    
                    (self.meter_protocol, self.num_phases) = {'10':('QB6', 6), 
                                                                                        '11':('QB4', 4),
                                                                                        '12':('QB1', 1)} [self.meter_serno[0:2]]
                except Exception as e:
                    
                    raise MeterError('Invalid meter type in serno: {}\n{}'.format(self.meter_serno, e))
                
            else:
                
                raise MeterError('Could not contact meter')
                
        else:                                                                                       # Meter is legacy family, use serial port and ascii commands to read
    ...
    def wait_pulses(self, timeout_secs, pulse_qty):
        end_time = datetime.datetime.now() + datetime.timedelta(seconds=timeout_secs)
        while datetime.datetime.now() < end_time:
            self.pulse_test.get_test_status()
            self.pulse_test.get_test_data()
            
            remaining_pulses = ''
            test_finished = True
            for i in range(self.num_phases):                
                remaining_pulse_count = pulse_qty - self.pulse_test.current[i].meter
                if remaining_pulse_count > 0:
                    test_finished = False
                remaining_pulses +='m{}:{} '.format(i+1,  remaining_pulse_count)
                
            Q_logger.info('waiting for pulses, status =  {}, pulses remaining: {}'.format(self.pulse_test.test_status, 
                                                                                                                                                        remaining_pulses))
                                                                                                                                                        
            lw.volatilemessage('Waiting for pulses from {} meter points:  {}'.format(self.num_phases, 
                                            remaining_pulses))
        ...
        lw.volatilemessage('{} waiting for pulses'.format('Done' if test_finished else 'Timeout'))
        return
